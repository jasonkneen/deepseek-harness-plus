 ...
 * @param continuation - whether the run persists its session; only then does the result carry `continuationId`.
...
export async function consumeClaudeQuery(
  ...
  return {
    ...
    ...continuation && continuationId !== undefined ? { continuationId } : {},
  }
}
...
/** One harness effort id from {@link CLAUDE_EFFORTS}. */
export type ClaudeEffort = (typeof CLAUDE_EFFORTS)[number]

/**
 * Map a harness effort id to the SDK's `effort`/`thinking` options.
 * @param effort - harness effort id, or `undefined` for the SDK default.
 * @returns the SDK option subset selecting that effort; empty for the default.
 */
 ...
 * @param continueFrom - engine session id to resume, or `undefined` for a fresh session.
 * @param model - explicit child model selection, or `undefined` for the native default.
 * @param effort - harness reasoning-effort id, or `undefined` for the SDK default.
 * @returns options that inherit native settings while fixing persistence, permissions, and user questions.
 */
...
export async function startClaudeCodeRun(
  ...
  const result = settleRunResult({
    attempt: async () => {
      try {
        return await consumeClaudeQuery(publishedQuery, channel, spec.continuation, () => {
