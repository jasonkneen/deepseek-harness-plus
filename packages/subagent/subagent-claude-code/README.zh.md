...
本包是可选的 Profile Bundle。将它安装进目标 Profile 后重启该 Profile；安装会把锁定的 Agent SDK 与一个兼容的平台 CLI 载荷带入该 Profile，而声明的 `cordis.patch.yml` 层只注册休眠的 `claude-code` Host provider，不会启动任何 Claude 进程。移除该包后，下一次 Profile 启动会撤回这一 provider 及其私有运行时闭包。
...
- **除非 `continuation: true`，每次运行均新建一个 query 和一个进程**：启用续接后，运行会持久化并恢复原生会话（Claude `resume`）；默认的一次性姿态是全新 query，不做产品会话持久化。query 运行期间，实时文本增量通过 `SubagentRun.updates` 流式输出。
