...
| `model` | 省略 | 可选的模型名，会随 `thread/start` 发送；省略时仍由原生项目与用户设置选择模型。 |
...
| `continuation` | `false` | 是否持久化线程并恢复早期对话（`continueFrom` 走 `thread/resume`）。持久线程会写入原生 Codex home；默认的一次性姿态保持线程为 ephemeral。 |
...
生产环境会解析锁定的 `@openai/codex@0.149.1` 依赖所声明的 `codex` bin，并使用当前 Node 可执行文件启动该 JavaScript wrapper。Wrapper 会选择匹配的原生平台载荷；提供方既不检查也不回退 `PATH` 中的宿主 `codex`。父会话 cwd、`HOME` 与 `CODEX_HOME` 继续让原生 Codex 配置和身份验证保持权威，而提供方只覆盖选定线程的 model／approval／reviewer／sandbox 字段。省略 `model` 时，仍由原生项目与用户设置选择模型。其他项目、provider、MCP、hook、skill 与账户设置仍由原生机制负责。本插件不创建 `CODEX_HOME`、不执行登录，也不探测账户。子进程 seam 会先移除具有凭证特征的环境变量，再应用显式 `env` 覆盖。
...
- **除非 `continuation: true`，每次运行均新建一个进程、一个线程和一个轮次**：启用续接后，线程会持久化并可恢复（`thread/resume`）；默认的一次性姿态保持线程为 ephemeral。轮次运行期间，实时 `item/agentMessage/delta` 文本通过 `SubagentRun.updates` 流式输出。
