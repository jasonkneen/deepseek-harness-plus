...
路由与输入校验完成后，辅助模块会在模型分发前将确切的可分发请求记录为仅写入日志的 `session/title-llm-request` 事件。该事件不进入派生模型历史；后续模型调用失败时仍会保留，而校验失败不会创建事件。请求用途会使 DeepSeek 标题调用禁用思考，其他适配器则负责自身的用途专用行为。
