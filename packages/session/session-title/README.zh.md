...
标题来自三个来源，最新者胜出。内置回退在配置上限内从第一条符合条件用户消息的开头若干词派生；已注册提供方对符合条件的消息生成标题；显式 `rename()` 接受用户提供的标题。只有追加来源的人工 `user/message` 文本块会触发自动工作，空提示词或非文本提示词会等待后续符合条件的输入。同会话编辑会取消待执行或正在运行的自动工作，但不会安排新的 revision，因此已有标题保持不变；若后续普通提示词触发新的提供方 revision，其消息快照会跟随当前对话代次，并排除被早先编辑隐藏的提示词。用户来源的最新标题会钉住会话——后续用户消息不再安排自动修订，显式 `refresh()` 仍是有意的解钉手段。
...
`get(session)` 从活跃或回放会话读取折叠出的最新标题，`foldSessionTitle(events)` 是对日志的纯折叠。服务要求 `ctx.sessionProjections` 并注册两个单元：客户端可见的 `title` 单元（供客户端列表行使用的已接受标题字符串）和仅供 host 使用的 `titleInput` 单元——后者折叠第一条与最新一条追加来源合格消息及其计数，使自动调度通过 `stateOf()` 达到 O(1)。回退与提供方生成会在执行时扫描日志，以选择当前对话代次。显式 `refresh(session)` 在需要时物化回退，然后对这些当前合格消息显式运行已注册提供方。
