...
describe('SessionTitleService Provider lifecycle', () => {
  ...
  it('does not regenerate for an edit but excludes replaced prompts from a later revision', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionProjectionRegistry)
    await ctx.plugin(SessionTitleService, CONFIG)
    const requests: SessionTitleProviderRequest[] = []
    ctx.sessionTitle.register({
      id: SessionTitleProviderId('edit-title'),
      automatic: 'all-prompts',
      async generate(request) {
        requests.push(request)
        return { title: `Title ${String(requests.length)}`, messageSeqs: request.messages.map(message => message.seq) }
      },
    })
    const session = ctx.sessions.create(SessionId('edit-title-session'))
    const turnStart = session.append('turn/start', { turn: 1 })
    const original = appendHumanPrompt(session, 'original prompt')
    await settle()
    appendRoute(session)
    await settle()
    const oldEnd = session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
    expect(requests).toHaveLength(1)

    session.append('turn/start', { turn: 2 })
    const edited = session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'edited prompt' }], source: { kind: 'user' },
    }), {
      surfaceOp: { op: 'replace', start: original.seq, end: original.seq },
      sourceEventSeqs: [original.seq],
      conversationOp: { op: 'replace', start: turnStart.seq, end: oldEnd.seq },
    })
    await settle()
    appendRoute(session, 'change')
    await settle()
    ...
    await ctx.sessionTitle.refresh(session)
    expect(requests).toHaveLength(2)
    expect(requests[1]?.messages.map(message => message.seq)).toEqual([edited.seq])

    session.append('turn/start', { turn: 3 })
    const later = appendHumanPrompt(session, 'later prompt')
    await settle()
    appendRoute(session, 'change')
    await settle()

    expect(requests).toHaveLength(3)
    expect(requests[2]?.messages.map(message => message.seq)).toEqual([edited.seq, later.seq])
  })

  it('cancels an older automatic title generation when an edit replacement commits', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionProjectionRegistry)
    await ctx.plugin(SessionTitleService, CONFIG)
    ...
    const requests: SessionTitleProviderRequest[] = []
    ctx.sessionTitle.register({
      id: SessionTitleProviderId('edit-cancel'),
      automatic: 'first-prompt',
      generate(request) {
        requests.push(request)
    ...
    const session = ctx.sessions.create(SessionId('edit-cancel-session'))
    const turnStart = session.append('turn/start', { turn: 1 })
    const original = appendHumanPrompt(session, 'original prompt')
    await settle()
    appendRoute(session)
    ...
    appendRoute(session, 'change')
    pending.resolve({ title: 'stale original title', messageSeqs: [original.seq] })
    await settle()

    expect(requests).toHaveLength(1)
    expect(ctx.sessionTitle.get(session)).toMatchObject({
      title: 'original prompt',
      messageSeqs: [original.seq],
      source: { kind: 'fallback' },
    })
    expect(edited.seq).toBeGreaterThan(oldEnd.seq)
  })
