...
Chat 会在历史前插与 renderer 重新挂载时恢复语义锚点。原始滚动事件会被节流；内容尺寸变化会先根据新底部结算任何待处理采样，然后 `ResizeObserver` 仅在 observed-top ledger 仍将底部归给跟随时追随它。已贴底的尺寸变化跟随无需读取行几何就选中最后一个已加载 Turn。读者离开底部后，高度变化会保持顶部位置，再由阅读线几何选择活跃 Turn。轮次导航预览位于 Markdown 代码块粘性头栏上方，而导航外框始终处于 composer 上方的 transcript 区域内（[读者滚动归因](../../../.agents/notes/implemented/bug-fix/2026-08-06-reader-scroll-attribution-observed-top-ledger.zh.md)、[已加载 Turn 导航](../../../.agents/notes/implemented/feature/2026-08-25-loaded-turn-chat-navigation.zh.md)）。
