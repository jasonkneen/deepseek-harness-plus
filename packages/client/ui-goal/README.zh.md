...
- **不依赖 preset 的 host 状态**——把 active 会话切换到 `minimal` 时，host 所拥有的 goal 保持不变；`/goal` 与 goal 工具会消失，但条带仍允许用户 edit、pause、resume 或 clear 该 goal。
