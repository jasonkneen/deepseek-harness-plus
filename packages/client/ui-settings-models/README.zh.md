...
Host 公布 `subagent-model-selection` settings namespace 时，Models 还会在提供方行上方渲染本地化开关。它默认关闭，只通过 `settings.update` 携带 namespace revision 写入 `{ enabled }`。Host 会在组合新的顶层 Session 时读取它；修改设置不会重新配置运行中的 Session，而子 Session 会继承父级已记录的决定。
