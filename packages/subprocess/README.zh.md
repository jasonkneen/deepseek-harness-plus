...
| [`subprocess-local`](subprocess-local/README.zh.md)（`@deepseek-ai/dsh-subprocess-local`） | 无 | 本地 Service Provider：native 与 fallback managed range、有界收集／spill、`node-pty`、前台／会话检查、信号发送，以及先终止再等待退出的 dispose（资源释放） |
