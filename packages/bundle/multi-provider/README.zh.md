...
以 profile 组合包形式交付的多 provider 包：[`cordis.patch.yml`](cordis.patch.yml) 是叠在 [`dsh-base`](../base/README.zh.md) 之上的可选层，在休眠挂载的 [`dsh-llm-pi-ai`](../../llm/llm-pi-ai/README.zh.md) 适配器上激活第三方模型 provider，并组合 [Claude Code](../../subagent/subagent-claude-code/README.zh.md) 与 [Codex](../../subagent/subagent-codex/README.zh.md) 委派后端。把 `@deepseek-ai/dsh-multi-provider` 加到 profile 的 `dsh.profile.bundles`（位于 `@deepseek-ai/dsh-base` 之后）即可启用整个包；移除即可整体卸载。该包没有运行时 API；profile 组合器通过 manifest 的 `dsh.bundle.patch` 字段解析 patch，绝不通过代码。
...
可运行的参考组合是 [`examples/multi-provider`](../../../examples/multi-provider/README.zh.md)：它在 agent spine 之上启动该包，无密钥即可验证路由注册，有密钥时对每个 provider 运行一次真实任务。
