...
describe('agent/pre-step', () => {
  it('supports following-only, surface-only, and empty Agent send options', async () => {
    const adapter = new MockAdapter([textResponse('one'), textResponse('two'), textResponse('three')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('send-options'), { provider: 'mock', model: 'mock' })
    const first = createUserMessage({ content: [{ type: 'text', text: 'first' }], source: { kind: 'user' } })
    const context = createUserMessage({
      content: [{ type: 'text', text: 'following' }], source: { kind: 'plugin', plugin: 'test' },
    })

    let idle = waitForIdle(ctx, agent)
    agent.send(first, 'next-turn', true, { followingMessages: [context] })
    ...
    agent.send(second, 'next-turn', true, { surfaceIntent: { surfaceOp: 'append' } })
    await idle
    expect(events(agent).find(event => event.type === 'user/message' && event.data.id === second.id))
      .toMatchObject({ surfaceOp: 'append' })

    const third = createUserMessage({ content: [{ type: 'text', text: 'third' }], source: { kind: 'user' } })
    idle = waitForIdle(ctx, agent)
    agent.send(third, 'next-turn', true, { followingMessages: [] })
    await idle
    expect(adapter.requests).toHaveLength(3)
  })

  it('records a claimed message with its durable replacement intent after listener rewrites', async () => {
    const adapter = new MockAdapter([textResponse('old answer'), textResponse('new answer')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('surface-intent'), { provider: 'mock', model: 'mock' })

    send(agent, 'original')
    await waitForIdle(ctx, agent)
    const before = events(agent)
    const original = before.find(event => event.type === 'user/message')
    const oldAnswer = before.find(event => event.type === 'assistant/message')
    const turnStart = before.find(event => event.type === 'turn/start')
    const turnEnd = before.findLast(event => event.type === 'turn/end')
    if (original?.type !== 'user/message' || oldAnswer?.type !== 'assistant/message'
      || turnStart?.type !== 'turn/start' || turnEnd?.type !== 'turn/end') {
      throw new Error('fixture did not complete its first turn')
    }
    ...
    const surfaceIntent = {
      surfaceOp: { op: 'replace' as const, start: original.seq, end: oldAnswer.seq },
      sourceEventSeqs: [original.seq, oldAnswer.seq],
      conversationOp: { op: 'replace' as const, start: turnStart.seq, end: turnEnd.seq },
    }
    ctx.on('agent/pre-step', async ({ surfaceIntents }, next) => {
      expect(surfaceIntents?.get(edited.id)).toEqual(surfaceIntent)
      const decision = await next()
      if (decision.kind === 'reject') return decision
      return {
        ...decision,
        messages: decision.messages.map(message => message.id === edited.id
          ? { ...message, content: [{ type: 'text', text: 'rewritten edit' }] }
          : message),
      }
    })

    const idle = waitForIdle(ctx, agent)
    agent.send(edited, 'next-turn', true, {
      position: 'front',
    ...
    const replacement = events(agent).find(event =>
      event.type === 'user/message' && event.data.id === edited.id)
    expect(replacement).toMatchObject({
      type: 'user/message',
      data: { content: [{ type: 'text', text: 'rewritten edit' }] },
      surfaceOp: { op: 'replace', start: original.seq, end: oldAnswer.seq },
      sourceEventSeqs: [original.seq, oldAnswer.seq],
      conversationOp: { op: 'replace', start: turnStart.seq, end: turnEnd.seq },
    })
    expect(adapter.requests[1]!.messages).toMatchObject([
      { content: [{ type: 'text', text: 'rewritten edit' }] },
      { content: [{ type: 'text', text: 'preserved context' }] },
    ])
    expect(JSON.stringify(adapter.requests[1]!.messages)).not.toContain('original')
    expect(JSON.stringify(adapter.requests[1]!.messages)).not.toContain('old answer')
  })
