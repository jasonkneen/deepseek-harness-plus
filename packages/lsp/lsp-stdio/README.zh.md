...
- 协议 shutdown 失败后，调用 subprocess provider 的终止过程，并通过 `waitForExit()` 等待同一个 managed range。信号投递与观察失败归 provider 所有；LSP Host 只拥有协议优先的拆卸过程与最终完全停稳等待。
...
| `killGraceMs` | `2000` | 提供给 subprocess 终止与输出排空的宽限期。 |
