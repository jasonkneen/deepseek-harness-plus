...
import * as EngineInvariant from '@deepseek-ai/dsh-engine-session-demo/invariant'

describe('engine-session-demo invariant companion', () => {
  it('registers against the invariant service and disposes', async () => {
    const ctx = new Context()
    await ctx.plugin(InvariantRegistry)
    const instance = await ctx.plugin(EngineInvariant)
