...
工具根据 `ToolExecution.exec.agent` 派生调用方权限。跨会话访问要求目标和调用方会话的 `cwd` 值严格相等；没有 `cwd` 的调用方只能检查自己。搜索绝不公开分页控制项或模型可控的结果上限。取消会保留调用方原因，时间戳过滤器要求显式 `Z` 或数字偏移，并包含两个端点。
...
提供方失败会记录可用的诊断信息，但向模型返回固定的 `SESSION_QUERY_TOOL_FAILED` 代码和消息。本地参数校验和授权错误保留精确的工具自有消息。
