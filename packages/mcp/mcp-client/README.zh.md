...
| `serverName` | 两者 | 是 | 该服务器面向模型工具名称的 namespace；`[A-Za-z0-9_-]{1,32}`，在单个 registration scope 内唯一 |
...
- 同一个存活 registration scope 内重复的 `serverName` 会使后加载的插件实例失败。独立 Agent scope 的工具和传输彼此隔离，因此可以复用同一 namespace。
