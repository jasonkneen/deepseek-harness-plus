...
Electron 通过内置的上游 Node.js 进程启动已安装的 dsh 包。一元 RPC、Remote stream 与版本匹配的客户端资源经子进程载体和安全的 `dsh-app://` 协议传输，因此桌面组合不会开放 Web server 或 loopback 端口。只有壳自有 UI 能通过内置 pnpm 及其私有 `$DSH_HOME/desktop/pnpm/store` 执行插件事务。
