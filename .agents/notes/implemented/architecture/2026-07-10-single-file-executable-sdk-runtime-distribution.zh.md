...
- [`apps/cli`](../../../../apps/cli/README.zh.md)（`@deepseek-ai/dsh`）：打包后的应用入口；其 `sdk` profile 挂载 `dsh-sdk-jsonrpc-server`，CLI 负责环境分层、profile 组合、stdin／signal 关闭与进程退出。在正常参数解析之前，打包后的 CLI 会识别一个保留的私有 runner 参数并转入 `dsh-subprocess-local`，使 native subprocess containment 可以重新进入同一个可执行文件，而不假定 `process.execPath` 是通用 Node 二进制。
